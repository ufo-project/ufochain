...
namespace
{
    ...
    {
        string jobID;
        ufo::Merkle::Hash input;
        ufo::Block::PoW pow;
        ufo::IExternalPOW::BlockFound callback;
    };

    using SolutionCallback = function<void(Job&& job)>;

    class WorkProvider : public ufoMiner::minerBridge
    {
    public:
        ...
        void handleSolution(int64_t &workId, uint64_t &nonce, vector<uint32_t> &indices, uint32_t difficulty) override
        {
            ...
            ufo::Block::PoW::NonceType t((const uint8_t*)&nonce);
            job.pow.m_Nonce = t;
            job.pow.m_Difficulty = ufo::Difficulty(difficulty);
