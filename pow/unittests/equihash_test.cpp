...
int main()
{
    ...
    {
      cout << "Test Difficulty...\n";
      ufo::Block::PoW pow;

      for (int i = 0; i <= 0x00ffffff; i = i + 0x0000ffff) {
        pow.m_Difficulty = i;
        auto floatDiff = pow.m_Difficulty.ToFloat();
        cout << "Difficulty:" << i << " |  " << floatDiff << endl;
      }
    }
    {
        cout << "Test PoW...\n";
        uint8_t pDigest[] = { 1, 2, 3, 4, 56 };
...

        {
          //  pow.Solve(pInput, sizeof(pInput));
            ...
            WALLET_CHECK(pow.IsValid(pDigest, sizeof(pDigest),0));
        }
