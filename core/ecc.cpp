...
//#	endif // __linux__
	...
	void Hash::Processor::Write(const ufo::Blob& v)
	{
		Write(v.p, v.n);
	}
	...
	void NonceGenerator::WriteIkm(const ufo::Blob& b)
	{
		assert(m_bFirstTime);
	...
	{
		NonceGenerator nonceGen1("ufo-HKdf");
		nonceGen1 << hv;
	...
	{
		NonceGenerator nonceGen("ufo-Schnorr");
	...
	namespace RangeProof
	{
		...
		{
			NonceGenerator nonceGen("ufo-psig");
		...
		void Public::get_Msg(Hash::Value& hv, Oracle& oracle) const
		{
			oracle
				<< m_Value
				<< ufo::Blob(&m_Recovery, sizeof(m_Recovery))
				>> hv;
		}
