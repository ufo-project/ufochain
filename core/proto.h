...
#define UfoNodeMsg_NewTip(macro) \
    macro(Block::SystemState::Full, Description)

#define UfoNodeMsg_GetHdr(macro) \
    macro(Block::SystemState::ID, ID)

#define UfoNodeMsg_Hdr(macro) \
    macro(Block::SystemState::Full, Description)

#define UfoNodeMsg_GetHdrPack(macro) \
    macro(Block::SystemState::ID, Top) \
    macro(uint32_t, Count)

#define UfoNodeMsg_HdrPack(macro) \
    macro(Block::SystemState::Sequence::Prefix, Prefix) \
    macro(std::vector<Block::SystemState::Sequence::Element>, vElements)

#define UfoNodeMsg_DataMissing(macro)

#define UfoNodeMsg_Status(macro) \
    macro(uint8_t, Value)

#define UfoNodeMsg_GetBody(macro) \
    macro(Block::SystemState::ID, ID)

#define UfoNodeMsg_GetBodyPack(macro) \
    macro(Block::SystemState::ID, Top) \
    macro(uint8_t, FlagP) \
    macro(uint8_t, FlagE) \
...
#define UfoNodeMsg_Body(macro) \
    macro(BodyBuffers, Body)

#define UfoNodeMsg_BodyPack(macro) \
    macro(std::vector<BodyBuffers>, Bodies)

#define UfoNodeMsg_GetProofState(macro) \
    macro(Height, Height)

#define UfoNodeMsg_GetCommonState(macro) \
    macro(std::vector<Block::SystemState::ID>, IDs)

#define UfoNodeMsg_GetProofKernel(macro) \
    macro(Merkle::Hash, ID)

#define UfoNodeMsg_GetProofKernel2(macro) \
    macro(Merkle::Hash, ID) \
    macro(bool, Fetch)

#define UfoNodeMsg_GetProofUtxo(macro) \
...
#define UfoNodeMsg_GetProofChainWork(macro) \
    macro(Difficulty::Raw, LowerBound)

#define UfoNodeMsg_ProofKernel(macro) \
    macro(TxKernel::LongProof, Proof)

#define UfoNodeMsg_ProofKernel2(macro) \
    macro(Merkle::Proof, Proof) \
    macro(Height, Height) \
    macro(TxKernel::Ptr, Kernel)

#define UfoNodeMsg_ProofUtxo(macro) \
    macro(std::vector<Input::Proof>, Proofs)

#define UfoNodeMsg_ProofState(macro) \
    macro(Merkle::HardProof, Proof)

#define UfoNodeMsg_ProofCommonState(macro) \
    macro(Block::SystemState::ID, ID) \
    macro(Merkle::HardProof, Proof)

#define UfoNodeMsg_ProofChainWork(macro) \
    macro(Block::ChainWorkProof, Proof)

#define UfoNodeMsg_Login0(macro) \
    macro(ECC::Hash::Value, CfgChecksum) \
    macro(uint8_t, Flags)

#define UfoNodeMsg_Login(macro) \
    macro(std::vector<ECC::Hash::Value>, Cfgs) \
    macro(uint32_t, Flags)

#define UfoNodeMsg_Ping(macro)
#define UfoNodeMsg_Pong(macro)

#define UfoNodeMsg_NewTransaction(macro) \
    macro(Transaction::Ptr, Transaction) \
    macro(bool, Fluff)

#define UfoNodeMsg_HaveTransaction(macro) \
    macro(Transaction::KeyType, ID)

#define UfoNodeMsg_GetTransaction(macro) \
    macro(Transaction::KeyType, ID)

#define UfoNodeMsg_Bye(macro) \
    macro(uint8_t, Reason)

#define UfoNodeMsg_PeerInfoSelf(macro) \
    macro(uint16_t, Port)

#define UfoNodeMsg_PeerInfo(macro) \
    macro(PeerID, ID) \
    macro(io::Address, LastAddr)

#define UfoNodeMsg_GetTime(macro)

#define UfoNodeMsg_Time(macro) \
    macro(Timestamp, Value)

#define UfoNodeMsg_GetExternalAddr(macro)

#define UfoNodeMsg_ExternalAddr(macro) \
    macro(uint32_t, Value)

#define UfoNodeMsg_BbsMsg(macro) \
    macro(BbsChannel, Channel) \
    macro(Timestamp, TimePosted) \
    macro(ByteBuffer, Message) \
    macro(Bbs::NonceType, Nonce)

#define UfoNodeMsg_BbsHaveMsg(macro) \
    macro(BbsMsgID, Key)

#define UfoNodeMsg_BbsGetMsg(macro) \
    macro(BbsMsgID, Key)

#define UfoNodeMsg_BbsSubscribe(macro) \
    macro(BbsChannel, Channel) \
    macro(Timestamp, TimeFrom) \
    macro(bool, On)

#define UfoNodeMsg_BbsResetSync(macro) \
    macro(Timestamp, TimeFrom)

#define UfoNodeMsg_SChannelInitiate(macro) \
    macro(ECC::uintBig, NoncePub)

#define UfoNodeMsg_SChannelReady(macro)

#define UfoNodeMsg_Authentication(macro) \
    macro(PeerID, ID) \
    macro(uint8_t, IDType) \
    macro(ECC::Signature, Sig)

#define UfoNodeMsg_GetUtxoEvents(macro) \
    macro(Height, HeightMin)

#define UfoNodeMsg_UtxoEvents(macro) \
    macro(std::vector<UtxoEvent>, Events)

#define UfoNodeMsg_GetBlockFinalization(macro) \
    macro(Height, Height) \
    macro(Amount, Fees)

#define UfoNodeMsg_BlockFinalization(macro) \
    macro(Transaction::Ptr, Value)

#define UfoNodeMsgsAll(macro) \
    /* general msgs */ \
    macro(0x00, Login0) \
    macro(0x01, Bye) \
    macro(0x02, Ping) \
...
#define THE_MACRO1(code, msg) \
    ...
    { \
        ...
        UfoNodeMsg_##msg(THE_MACRO2) \
        template <typename Archive> void serialize(Archive& ar) { ar UfoNodeMsg_##msg(THE_MACRO3); } \
        msg(Zero_ = Zero) { UfoNodeMsg_##msg(THE_MACRO4) } /* default c'tor, zero-init everything */ \
        msg(Uninitialized_) { } /* don't init members */ \
        msg(UfoNodeMsg_##msg(THE_MACRO5) Unused_ = Unused) { UfoNodeMsg_##msg(THE_MACRO6) } /* explicit init */ \
    }; \
    struct msg##_NoInit :public msg { \
        msg##_NoInit() :msg(Uninitialized) {} \
    }; \

    UfoNodeMsgsAll(THE_MACRO1)
#undef THE_MACRO1
#undef THE_MACRO2
#undef THE_MACRO3
#undef THE_MACRO4
