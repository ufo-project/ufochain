...
const ufo::Height g_hFork = 3; // whatever
...
void TestShifted(const ufo::uintBig_t<n0>& x0, const ufo::uintBig_t<n1>& x1, int nShift)
{
	TestShifted2(x0.m_pData, x0.nBytes, x1.m_pData, x1.nBytes, nShift);
}

template <uint32_t n0, uint32_t n1>
void TestShifts(const ufo::uintBig_t<n0>& src, ufo::uintBig_t<n0>& src2, ufo::uintBig_t<n1>& trg, int nShift)
{
	src2 = src;
...
void TestUintBig()
{
	...
	{
		ufo::uintBig_t<32> a;
		ufo::uintBig_t<32 - 8> b;
		ufo::uintBig_t<32 + 8> c;
		ufo::uintBig_t<32> d;
...
void TestCommitments()
{
	...
	for (uint8_t iCycle = 0; iCycle < 2; iCycle++)
	{
		...
		ufo::SwitchCommitment().Recover(sigma, kdf, kidv);
		sigma = -sigma;
		sigma += comm;
		verify_test(sigma == Zero);
	}
...
void TestRangeProof(bool bCustomTag)
{
	...
	AssetTag tag;
	tag.m_hGen = ufo::SwitchCommitment(&aid).m_hGen;
	...
	WriteSizeSerialized("In-Utxo", ufo::Input());
...
void TestMultiSigOutput()
{
    ...
    ufo::Key::IKdf::Ptr pKdf_A;
    ufo::Key::IKdf::Ptr pKdf_B;
    uintBig secretB;
    uintBig secretA;
    SetRandom(secretA);
    SetRandom(secretB);
    ...
    ufo::SwitchCommitment switchCommitment;
    ...
    ufo::Output::GenerateSeedKid(creatorParamsB.m_Seed.V, outp.m_Commitment, *pKdf_B);
    ...
    // create Input
    std::unique_ptr<ufo::Input> pInput(new ufo::Input);
    ...
    kidv.m_Value = amount;
    Scalar::Native k;
    ufo::SwitchCommitment(nullptr).Create(k, pInput->m_Commitment, *pKdf_A, kidv);
    offset = k;

    // output
    std::unique_ptr<ufo::Output> pOutput(new ufo::Output);
	*pOutput = outp;
    {
        ECC::Point::Native comm;
    ...
    std::unique_ptr<ufo::TxKernel> pKernel(new ufo::TxKernel);
    pKernel->m_Fee = 0;
    ...
    ufo::TxBase::Context::Params pars;
    ufo::TxBase::Context context(pars);
...
struct TransactionMaker
{
	...
	struct Peer
	{
		...
		void AddInput(ufo::Transaction& t, Amount val, Key::IKdf& kdf, const ufo::AssetID* pAssetID = nullptr, bool is_trezor_debug = false)
		{
			std::unique_ptr<ufo::Input> pInp(new ufo::Input);
			...
			Scalar::Native k;
			ufo::SwitchCommitment(pAssetID).Create(k, pInp->m_Commitment, kdf, kidv);

			t.m_vInputs.push_back(std::move(pInp));
			m_k += k;
		}

		void AddOutput(ufo::Transaction& t, Amount val, Key::IKdf& kdf, const ufo::AssetID* pAssetID = nullptr, bool is_trezor_debug = false)
		{
			std::unique_ptr<ufo::Output> pOut(new ufo::Output);
	...
	void CoSignKernel(ufo::TxKernel& krn, const Hash::Value& hvLockImage, bool is_trezor_debug = false)
	{
	...
	void CreateTxKernel(std::vector<ufo::TxKernel::Ptr>& lstTrg, Amount fee, std::vector<ufo::TxKernel::Ptr>& lstNested, bool bEmitCustomTag, bool bNested, bool is_trezor_debug = false)
	{
		std::unique_ptr<ufo::TxKernel> pKrn(new ufo::TxKernel);
		pKrn->m_Fee = fee;
		pKrn->m_CanEmbed = bNested;
		pKrn->m_vNested.swap(lstNested);

		// hashlock
		pKrn->m_pHashLock.reset(new ufo::TxKernel::HashLock);
		...
		if (bEmitCustomTag)
		{
			...
			ufo::proto::Sk2Pk(aid, skAsset);

			if (ufo::Rules::get().CA.Deposit)
			...
			std::unique_ptr<ufo::TxKernel> pKrnEmission(new ufo::TxKernel);
...
void TestTransaction()
{
	...
	std::vector<ufo::TxKernel::Ptr> lstNested, lstDummy;
	...
	ufo::TxBase::Context::Params pars;
	ufo::TxBase::Context ctx(pars);
	ctx.m_Height.m_Min = g_hFork;
	verify_test(tm.m_Trans.IsValid(ctx));
	verify_test(ctx.m_Fee == ufo::AmountBig::Type(fee1 + fee2));
}
...
struct IHWWallet
{
	...
	void CreateInput(ufo::Input& res, const Key::IDV& kidv)
	{
	...
	virtual void CreateOutput(ufo::Output&, const Key::IDV&) = 0;

	struct TransactionData
		:public TransactionInOuts
	{
		// common kernel parameters
		ufo::HeightRange m_Height;
		ufo::Amount m_Fee;
		// aggregated data
	...
	:public IHWWallet
{
	...
	{
		Scalar::Native sk;
		ufo::SwitchCommitment().Create(sk, *m_pKdf, kidv);
		res += sk;
		dVal += kidv.m_Value;
	}
	...
	virtual void CreateOutput(ufo::Output& outp, const Key::IDV& kidv)  override
	{
		Scalar::Native sk;
...
struct MyWallet
{
	...
	void AddInp(Amount val)
	{
		...
		ufo::Input::Ptr pInp(new ufo::Input);
	...
	void AddOutp(Amount val)
	{
		...
		ufo::Output::Ptr pOutp(new ufo::Output);
...
void TestTransactionHW()
{
	...
	{
		ufo::AmountBig::Type fees;
		Point::Native exc;
		verify_test(mw1.m_Kernel.IsValid(g_hFork, fees, exc));
	}

	// merge txs
	ufo::Transaction txFull;
	ufo::TxVectors::Writer wtx(txFull, txFull);
	...
	ufo::TxKernel::Ptr pKrn(new ufo::TxKernel);
	*pKrn = mw1.m_Kernel;
...
void TestBbs()
{
	...
	ufo::PeerID publicAddr;

	SetRandom(privateAddr);
	ufo::proto::Sk2Pk(publicAddr, privateAddr);

	const char szMsg[] = "Hello, World!";

	SetRandom(nonce);
	ufo::ByteBuffer buf;
	verify_test(ufo::proto::Bbs::Encrypt(buf, publicAddr, nonce, szMsg, sizeof(szMsg)));

	uint8_t* p = &buf.at(0);
	uint32_t n = (uint32_t) buf.size();

	verify_test(ufo::proto::Bbs::Decrypt(p, n, privateAddr));
...
void TestRatio(const ufo::Difficulty& d0, const ufo::Difficulty& d1, double k)
{
...
void TestTreasury()
{
	...
	for (uint32_t i = 0; i < nPeers; i++)
	{
		...
		ufo::PeerID pid;
		Scalar::Native sk;
		ufo::Treasury::get_ID(pKdfs[i], pid, sk);

		// 2. Plan is created (2%, 3%, 4% of the total emission)
		ufo::Treasury::Entry* pE = tres.CreatePlan(pid, ufo::Rules::get().Emission.Value0 * (i + 2)/100, pars);
	...
	ufo::Treasury::Data data;
	data.m_sCustomMsg = msg;
	tres.Build(data);
	verify_test(!data.m_vGroups.empty());

	std::vector<ufo::Treasury::Data::Burst> vBursts = data.get_Bursts();
	...
	for (uint32_t i = 0; i < nPeers; i++)
	{
		std::vector<ufo::Treasury::Data::Coin> vCoins;
...
void TestTxKernel()
{
    ...
    std::vector<ufo::TxKernel::Ptr> lstNested;
    std::vector<ufo::TxKernel::Ptr> lstDummy;
...
int main()
{
	...
	ufo::Rules::get().pForks[1].m_Height = g_hFork;
	ECC::TestAll();
	ECC::RunBenchmark();
