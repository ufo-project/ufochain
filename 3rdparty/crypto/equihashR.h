...
#if defined(__ANDROID__) || !defined(UFO_USE_AVX)
...
class EhSolverCancelledException : public std::exception
{
    virtual const char* what() const throw() {
        return "UfoHash solver was cancelled";
    }
};

inline constexpr const size_t max(const size_t A, const size_t B) { return A > B ? A : B; }

inline constexpr size_t ufohash_solution_size(unsigned int N, unsigned int K) {
...
static EquihashR<150,5,0> UfoHashI;
static EquihashR<150,5,3> UfoHashII;


#define EhRInitialiseState(n, k, r, base_state)  \
     if (n == 150 && k == 5 && r == 0) { 				\
        UfoHashI.InitialiseState(base_state); 	\
    } else if (n == 150 && k == 5 && r == 3)) {       			\
        UfoHashII.InitialiseState(base_state);	\
    } else {                                 				\
...
#define EhRIsValidSolution(n, k, r, base_state, soln, ret)   	\
    if (n == 150 && k == 5 && r == 0) {                 	\
        ret = UfoHashI.IsValidSolution(base_state, soln); 	\
    } else if (n == 150 && k == 5 && r == 3)) {            	\
        ret = UfoHashII.IsValidSolution(base_state, soln);  	\
    } else {                                             	\
        throw std::invalid_argument("Unsupported Equihash parameters"); \
    }
