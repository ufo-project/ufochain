...
int EquihashR<N,K,R>::InitialiseState(eh_HashState& base_state)
{
    ...
    memcpy(personalization, "Ufo-PoW", 8);
                                   ...
                                   const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    for (std::shared_ptr<eh_trunc> partialSoln : partialSolns) {
        ...
        for (FullStepRow<FinalFullWidth> row : *X[K]) {
            ...
            assert(soln.size() == ufohash_solution_size(N, K));
            solns.insert(soln);
        }
        for (auto soln : solns) {
            if (validBlock(soln))
...
// Explicit instantiations for UfoHashI
template int EquihashR<N_UFO,K_UFO,0>::InitialiseState(eh_HashState& base_state);
template bool EquihashR<N_UFO,K_UFO,0>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln);
#ifdef ENABLE_MINING
template bool EquihashR<N_UFO,K_UFO,0>::OptimisedSolve(const eh_HashState& base_state,
                                             const std::function<bool(const std::vector<unsigned char>&)> validBlock,
                                             const std::function<bool(EhSolverCancelCheck)> cancelled);
#endif


// Explicit instantiations for UfoHashII
template int EquihashR<N_UFO,K_UFO,3>::InitialiseState(eh_HashState& base_state);
template bool EquihashR<N_UFO,K_UFO,3>::IsValidSolution(const eh_HashState& base_state, std::vector<unsigned char> soln);
#ifdef ENABLE_MINING
template bool EquihashR<N_UFO,K_UFO,3>::OptimisedSolve(const eh_HashState& base_state,
                                             const std::function<bool(const std::vector<unsigned char>&)> validBlock,
                                             const std::function<bool(EhSolverCancelCheck)> cancelled);
#endif
