...
#ifdef WIN32
		std::wstring sPathArg = ufo::Utf8toUtf16(sz);
#else // WIN32
...
void ufo::Crash::InstallHandler(const char* szLocation)
{
	if (szLocation)
	{
		std::wstring s = ufo::Utf8toUtf16(szLocation);
		size_t nLen = s.size();
...
void ufo::Crash::InstallHandler(const char*)
{
}

#endif // WIN32

void ufo::Crash::Induce(Type type)
{
	switch (type)
	{
	case StlInvalid:
