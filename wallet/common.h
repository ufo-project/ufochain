...
#define UFO_TX_FAILURE_REASON_MAP(MACRO) \
...
#define MACRO(name, code, _) name = code, 
        UFO_TX_FAILURE_REASON_MAP(MACRO)
#undef MACRO
    };
...
namespace std
{
    string to_string(const ufo::wallet::WalletID&);
    string to_string(const ufo::Merkle::Hash& hash);
    ...
    string to_string(const ufo::wallet::TxParameters&);
}
