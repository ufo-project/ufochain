...
namespace
{
    ...
    libbitcoin::chain::script CreateAtomicSwapContract(const ufo::wallet::BaseTransaction& tx, bool isBtcOwner, uint8_t addressVersion)
    {
        using namespace ufo;
        ...
        if (NoLeak<uintBig> secretPrivateKey; tx.GetParameter(TxParameterID::AtomicSwapSecretPrivateKey, secretPrivateKey.V, SubTxIndex::UFO_REDEEM_TX))
        {
        ...
        {
            Point publicKeyPoint = tx.GetMandatoryParameter<Point>(TxParameterID::AtomicSwapSecretPublicKey, SubTxIndex::UFO_REDEEM_TX);
...
{
    BitcoinSide::BitcoinSide(BaseTransaction& tx, bitcoin::IBridge::Ptr bitcoinBridge, bitcoin::ISettingsProvider& settingsProvider, bool isUfoSide)
        : m_tx(tx)
        , m_bitcoinBridge(bitcoinBridge)
        , m_settingsProvider(settingsProvider)
        , m_isBtcOwner(!isUfoSide)
    {
    ...
    bool BitcoinSide::HasEnoughTimeToProcessLockTx()
    {
        ...
        if (m_tx.GetParameter(TxParameterID::MaxHeight, lockTxMaxHeight, SubTxIndex::UFO_LOCK_TX))
        {
            Block::SystemState::Full systemState;
            if (m_tx.GetTip(systemState) && systemState.m_Height > lockTxMaxHeight - GetLockTxEstimatedTimeInUfoBlocks())
            {
                return false;
            }
        }
    ...
    uint32_t BitcoinSide::GetLockTxEstimatedTimeInUfoBlocks() const
    {
        // it's average value
        return 70;
    }
    ...
    void BitcoinSide::InitSecret()
    {
        ...
        m_tx.SetParameter(TxParameterID::AtomicSwapSecretPrivateKey, secretPrivateKey.V, false, UFO_REDEEM_TX);
    }
    ...
    void BitcoinSide::OnDumpPrivateKey(SubTxID subTxID, const bitcoin::IBridge::Error& error, const std::string& privateKey)
    {
        try
        {
            ...
            else
            {
                ...
                NoLeak<uintBig> secretPrivateKey;
                m_tx.GetParameter(TxParameterID::AtomicSwapSecretPrivateKey, secretPrivateKey.V, SubTxIndex::UFO_REDEEM_TX);
                libbitcoin::ec_secret secret;
                std::copy(std::begin(secretPrivateKey.V.m_pData), std::end(secretPrivateKey.V.m_pData), secret.begin());
