...
{
    template<>
    struct hash<pair<ufo::Amount, ufo::Amount>>
    {
        typedef pair<ufo::Amount, ufo::Amount> argument_type;
...
namespace ufo::wallet
{
    ...
    WalletDB::~WalletDB()
    {
        if (_db)
        {
            ...
            UFO_VERIFY(SQLITE_OK == sqlite3_close(_db));
            if (m_PrivateDB && _db != m_PrivateDB)
            {
                UFO_VERIFY(SQLITE_OK == sqlite3_close(m_PrivateDB));
                m_PrivateDB = nullptr;
            }
            _db = nullptr;
        }
    ...
    ufo::Key::IPKdf::Ptr WalletDB::get_OwnerKdf() const
    {
        return m_OwnerKdf;
    }
    ...
    namespace storage
    {
        ...
        WalletAddress createAddress(IWalletDB& walletDB, IPrivateKeyKeeper::Ptr keyKeeper)
        {
            ...
            newAddress.m_createTime = ufo::getTimestamp();
