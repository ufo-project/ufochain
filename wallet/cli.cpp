...
namespace
{
    ...
    int ShowWalletInfo(const IWalletDB::Ptr& walletDB, const po::variables_map& vm)
    {
        ...
        if (vm.count(cli::SWAP_TX_HISTORY))
        {
            ...
            for (auto& tx : txHistory)
            {
                ...
                storage::getTxParameter(*walletDB, tx.m_txId, wallet::kDefaultSubTxID, wallet::TxParameterID::AtomicSwapIsUfoSide, isUfoSide);
                ...
                stringstream ss;
                ss << (isUfoSide ? kUFO : to_string(swapCoin)) << " <--> " << (!isUfoSide ? kUFO : to_string(swapCoin));
        ...
        cout << boost::format(kCoinsTableHeadFormat)
                 ...
                 % boost::io::group(right, setw(columnWidths[1]), kUFO)
    ...
    int ExportPaymentProof(const IWalletDB::Ptr& walletDB, const po::variables_map& vm)
    {
        ...
        if (!res.empty())
        {
            ...
            ufo::to_hex(&sTxt.front(), res.data(), res.size());
    ...
    int ExportMinerKey(const po::variables_map& vm, const IWalletDB::Ptr& walletDB, const ufo::SecString& pass)
    {
    ...
    int ExportOwnerKey(const IWalletDB::Ptr& walletDB, const ufo::SecString& pass)
    {
    ...
    bool LoadBaseParamsForTX(const po::variables_map& vm, Amount& amount, Amount& fee, WalletID& receiverWalletID, bool checkFee, bool skipReceiverWalletID=false)
    {
        ...
        signedAmount *= Rules::Coin; // convert ufos to groths
    ...
    boost::optional<TxID> InitSwap(const po::variables_map& vm, const IWalletDB::Ptr& walletDB, IPrivateKeyKeeper::Ptr keyKeeper, Wallet& wallet, bool checkFee)
    {
        ...
        bool isUfoSide = (vm.count(cli::SWAP_UFO_SIDE) != 0);
        ...
        auto swapTxParameters = InitNewSwap(senderAddress.m_walletID, minHeight, amount, fee, swapCoin, swapAmount, isUfoSide);
        ...
        // print swap tx token
        {
            ...
            isUfoSide = !*swapTxParameters.GetParameter<bool>(TxParameterID::AtomicSwapIsUfoSide);
            ...
            swapTxParameters.SetParameter(TxParameterID::AtomicSwapIsUfoSide, isUfoSide);
            swapTxParameters.SetParameter(TxParameterID::IsSender, isUfoSide);
    ...
    boost::optional<TxID> AcceptSwap(const po::variables_map& vm, const IWalletDB::Ptr& walletDB, IPrivateKeyKeeper::Ptr keyKeeper, Wallet& wallet, bool checkFee)
    {
        ...
        auto swapTxParameters = ufo::wallet::ParseParameters(swapTxToken);

        // validate TxType and parameters
        auto transactionType = swapTxParameters->GetParameter<TxType>(TxParameterID::TransactionType);
        auto isUfoSide = swapTxParameters->GetParameter<bool>(TxParameterID::AtomicSwapIsUfoSide);
        auto swapCoin = swapTxParameters->GetParameter<AtomicSwapCoin>(TxParameterID::AtomicSwapCoin);
        auto ufoAmount = swapTxParameters->GetParameter<Amount>(TxParameterID::Amount);
        ...
        bool isValidToken = isUfoSide && swapCoin && ufoAmount && swapAmount && peerID && peerResponseTime;
        ...
        cout << " Swap conditions: " << "\n"
            ...
            << " Ufo amount:  " << PrintableAmount(*ufoAmount) << "\n"
        ...
        // TODO: check fee
        swapTxParameters->SetParameter(ufo::wallet::TxParameterID::Fee, ufo::Amount(cli::kMinimumFee));
        auto subTxID = isUfoSide ? ufo::wallet::SubTxIndex::REDEEM_TX : ufo::wallet::SubTxIndex::LOCK_TX;
        swapTxParameters->SetParameter(ufo::wallet::TxParameterID::Fee, swapFeeRate, subTxID);
...
#define LOG_FILES_PREFIX "wallet_"
        ...
        auto logger = ufo::Logger::create(logLevel, logLevel, fileLogLevel, LOG_FILES_PREFIX, path.string());
        ...
        try
        {
...

            {
...

                {
                    ...
                    {
                        if (!ufo::confirm_wallet_pass(pass))
                        {
