...
namespace ufo::wallet
{
    ...
    void SwapOffersBoard::publishOffer(const SwapOffer& offer) const
    {
        ...
        auto isUfoSide = offer.GetParameter<bool>(TxParameterID::AtomicSwapIsUfoSide);
        ...
        if (!swapCoin || !isUfoSide || !amount || !swapAmount || !responseTime || !minimalHeight)
        {
        ...
        LOG_INFO() << offer.m_txId << " Publish offer.\n\t"
                    << "isUfoSide: " << (*isUfoSide ? "false" : "true") << "\n\t"
        ...
        ufo::wallet::SwapOfferToken token(offer);
    ...
    void SwapOffersBoard::sendUpdateToNetwork(const TxID& offerID, const WalletID& publisherID, AtomicSwapCoin coin, SwapOfferStatus newStatus) const
    {
        ...
        ufo::wallet::SwapOfferToken token(SwapOffer(offerID, newStatus, publisherID, coin));
