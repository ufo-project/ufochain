...
namespace ufo::wallet
{
    LockTxBuilder::LockTxBuilder(BaseTransaction& tx, Amount amount, Amount fee)
        : BaseTxBuilder(tx, SubTxIndex::UFO_LOCK_TX, { amount }, fee)
    {
        Height minHeight = 0;
    ...
    bool LockTxBuilder::CreateSharedUTXOProofPart2(bool isUfoOwner)
    {
        // load peer part2
        ...
        // produce multisig
        if (!m_SharedProof.CoSign(GetSharedSeed(), GetSharedBlindingFactor(), GetProofCreatorParams(isUfoOwner), oracle, RangeProof::Confidential::Phase::Step2))
        {
            return false;
        }
    ...
    bool LockTxBuilder::CreateSharedUTXOProofPart3(bool isUfoOwner)
    {
        Output outp;
        outp.m_Commitment = GetSharedCommitment();
        ...
        if (isUfoOwner)
        {
            ...
            // finalize proof
            if (!m_SharedProof.CoSign(GetSharedSeed(), GetSharedBlindingFactor(), GetProofCreatorParams(isUfoOwner), oracle, RangeProof::Confidential::Phase::Finalize))
            {
                return false;
            }
        }
    ...
    void LockTxBuilder::LoadSharedParameters()
    {
        if (!m_Tx.GetParameter(TxParameterID::SharedBlindingFactor, m_SharedBlindingFactor, m_SubTxID))
        {
            ...
            m_Tx.SetParameter(TxParameterID::InputCoins, sharedInputs, static_cast<SubTxID>(SubTxIndex::UFO_REDEEM_TX));
            m_Tx.SetParameter(TxParameterID::InputCoins, sharedInputs, static_cast<SubTxID>(SubTxIndex::UFO_REFUND_TX));
    ...
    const ECC::RangeProof::CreatorParams& LockTxBuilder::GetProofCreatorParams(bool isUfoOwner)
    {
        if (!m_CreatorParams.is_initialized())
        {
            ...
            ECC::Hash::Processor()
                << (isUfoOwner ? publicSharedBlindingFactor : peerPublicSharedBlindingFactor)
                << (isUfoOwner ? peerPublicSharedBlindingFactor : publicSharedBlindingFactor)
