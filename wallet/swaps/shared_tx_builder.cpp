...
namespace ufo::wallet
{
    ...
    {
        return m_Tx.GetParameter(TxParameterID::SharedBlindingFactor, m_SharedBlindingFactor, SubTxIndex::UFO_LOCK_TX)
            && m_Tx.GetParameter(TxParameterID::PeerPublicSharedBlindingFactor, m_PeerPublicSharedBlindingFactor, SubTxIndex::UFO_LOCK_TX);
    }
    ...
    void SharedTxBuilder::InitMinHeight()
    {
        ...
        if (!m_Tx.GetParameter(TxParameterID::MinHeight, minHeight, m_SubTxID))
        {
            ...
            if (SubTxIndex::UFO_REFUND_TX == m_SubTxID)
            {
                minHeight += kUfoLockTimeInBlocks;
            }
