...
namespace ufo::wallet
{
    ...
    {
    public:
        LitecoinSide(BaseTransaction& tx, bitcoin::IBridge::Ptr bitcoinBridge, litecoin::ISettingsProvider& settingsProvider, bool isUfoSide);

        static bool CheckAmount(Amount amount, Amount feeRate);

    protected:

        uint32_t GetLockTxEstimatedTimeInUfoBlocks() const override;
