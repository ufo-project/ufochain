...
void TestSwapTransaction(bool isUfoOwnerStart, ufo::Height fork1Height)
{
    ...
    NodeObserver observer([&]()
    {
        ...
        if (cursor.m_Sid.m_Height == fork1Height + 5)
        {
            ...
            auto parameters = CreateSwapParameters()
                .SetParameter(TxParameterID::Amount, ufoAmount)
                .SetParameter(TxParameterID::Fee, ufoFee)
            ...
            if (isUfoOwnerStart)
            {
                parameters.SetParameter(TxParameterID::MyID, receiver.m_WalletID)
                    .SetParameter(TxParameterID::AtomicSwapIsUfoSide, false);
            ...
            else
            {
                parameters.SetParameter(TxParameterID::MyID, sender.m_WalletID)
                    .SetParameter(TxParameterID::AtomicSwapIsUfoSide, true);
    ...
    WALLET_CHECK(receiverCoins[0].m_ID.m_Value == ufoAmount - kMinFeeInGroth);
    ...
    storage::getTxParameter(*sender.m_WalletDB, txID, SubTxIndex::UFO_REDEEM_TX, TxParameterID::AtomicSwapSecretPrivateKey, senderSecretPrivateKey.V);
    NoLeak<uintBig> receiverSecretPrivateKey;
    storage::getTxParameter(*receiver.m_WalletDB, txID, SubTxIndex::UFO_REDEEM_TX, TxParameterID::AtomicSwapSecretPrivateKey, receiverSecretPrivateKey.V);
    WALLET_CHECK(senderSecretPrivateKey.V != Zero && senderSecretPrivateKey.V == receiverSecretPrivateKey.V);
}

void TestElectrumSwapTransaction(bool isUfoOwnerStart, ufo::Height fork1Height)
{
...
void TestSwapTransactionWithoutChange(bool isUfoOwnerStart)
{
    cout << "\nTesting atomic swap transaction...\n";
    ...
    if (isUfoOwnerStart)
    {
        auto parameters = InitNewSwap(receiver.m_WalletID, currentHeight, ufoAmount, ufoFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, false);
        receiver.m_Wallet.StartTransaction(parameters);
        txID = sender.m_Wallet.StartTransaction(AcceptSwapParameters(parameters, sender.m_WalletID));
    }
    else
    {
        auto parameters = InitNewSwap(sender.m_WalletID, currentHeight, ufoAmount, ufoFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, true);
        sender.m_Wallet.StartTransaction(parameters);
        txID = receiver.m_Wallet.StartTransaction(AcceptSwapParameters(parameters, receiver.m_WalletID));
    }
...
void TestSwapUfoRefundTransaction()
{
    cout << "\nAtomic swap: testing Ufo refund transaction...\n";
    ...
    eventToUpdate = io::AsyncEvent::create(*mainReactor, [&sender, &receiver, txID, &eventToUpdate, &node]()
    {
        if (receiver)
        {
            wallet::AtomicSwapTransaction::State txState = wallet::AtomicSwapTransaction::State::Initial;
            storage::getTxParameter(*receiver->m_WalletDB, txID, wallet::kDefaultSubTxID, wallet::TxParameterID::State, txState);
            if (txState == wallet::AtomicSwapTransaction::State::SendingUfoRedeemTX)
            {
                // delete receiver to simulate refund on Ufo side
                receiver.reset();
            }
            eventToUpdate->post();
        }
        else
        {
            wallet::AtomicSwapTransaction::State txState = wallet::AtomicSwapTransaction::State::Initial;
            storage::getTxParameter(*sender->m_WalletDB, txID, wallet::kDefaultSubTxID, wallet::TxParameterID::State, txState);
            if (txState != wallet::AtomicSwapTransaction::State::SendingUfoRefundTX)
            {
                // speed-up test
                node.AddBlock();
                eventToUpdate->post();
    ...
    // change of Ufo LockTx
    WALLET_CHECK(senderCoins[4].m_ID.m_Value == 100);
    WALLET_CHECK(senderCoins[4].m_status == Coin::Available);
    WALLET_CHECK(senderCoins[4].m_createTxId == txID);

    // Refund
    WALLET_CHECK(senderCoins[5].m_ID.m_Value == ufoAmount - kMinFeeInGroth);
    WALLET_CHECK(senderCoins[5].m_status == Coin::Available);
    WALLET_CHECK(senderCoins[5].m_createTxId == txID);
}

void TestElectrumSwapUfoRefundTransaction()
{
    cout << "\nAtomic swap: testing Ufo refund transaction on electrum...\n";
...
void ExpireByResponseTime(bool isUfoSide)
{
    // Simulate swap transaction without response from second side

    cout << "\nAtomic swap: testing expired transaction on " << (isUfoSide ? "Ufo" : "BTC") << " side...\n";
    ...
    TestNode node{ TestNode::NewBlockFunc(), kNodeStartHeight };
    Height currentHeight = node.m_Blockchain.m_mcm.m_vStates.size();
    auto swapParameters = InitNewSwap(receiverWalletID, currentHeight, ufoAmount, ufoFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, !isUfoSide, lifetime, responseTime);
...
void TestSwap120Blocks()
{
    ...
    NodeObserver observer([&]()
    {
        ...
        if (cursor.m_Sid.m_Height == minHeight)
        {
            ...
            auto parameters = InitNewSwap(receiver->m_WalletID, minHeight, ufoAmount, ufoFee, wallet::AtomicSwapCoin::Bitcoin, swapAmount, false);
            receiver->m_Wallet.StartTransaction(parameters);
            txID = sender->m_Wallet.StartTransaction(AcceptSwapParameters(parameters, sender->m_WalletID));
        }
    });
...
int main()
{
    ...
    auto logger = ufo::Logger::create(logLevel, logLevel);
    Rules::get().FakePoW = true;
    Rules::get().UpdateChecksum();
    ufo::Height fork1Height = 10;
    ...
    TestSwapCancelTransaction(true, wallet::AtomicSwapTransaction::State::BuildingUfoLockTX);
    TestSwapCancelTransaction(false, wallet::AtomicSwapTransaction::State::BuildingUfoLockTX);

    TestSwapCancelTransaction(true, wallet::AtomicSwapTransaction::State::BuildingUfoRedeemTX);
    TestSwapCancelTransaction(false, wallet::AtomicSwapTransaction::State::BuildingUfoRedeemTX);

    TestSwapCancelTransaction(true, wallet::AtomicSwapTransaction::State::BuildingUfoRefundTX);
    TestSwapCancelTransaction(false, wallet::AtomicSwapTransaction::State::BuildingUfoRefundTX);
    ...
    TestElectrumSwapUfoRefundTransaction();
#endif // UFO_TESTNET
