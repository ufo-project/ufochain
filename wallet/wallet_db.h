...
namespace ufo::wallet
{
    ...
    struct Coin
    {
        ...
        uint64_t m_sessionId;   // Used in the API to lock coins for specific session (see https://github.com/UfoMW/ufo/wiki/Ufo-wallet-protocol-API#tx_split)
    ...
    struct IWalletDB : IVariablesDB
    {
        ...
        virtual ufo::Key::IKdf::Ptr get_MasterKdf() const = 0;

        // Returns the Child Key Derivative Function (operates on secret keys)
		ufo::Key::IKdf::Ptr get_ChildKdf(const Key::IDV&) const;

        // Returns the Owner Key Derivative Function (operates on public keys)
        virtual ufo::Key::IPKdf::Ptr get_OwnerKdf() const = 0;
    ...
    public:
        ...
        ufo::Key::IKdf::Ptr get_MasterKdf() const override;
        ufo::Key::IPKdf::Ptr get_OwnerKdf() const override;
