...
struct KeyKeeper
{
    KeyKeeper(const ufo::WordList& words)
    {
        static_assert(sizeof(uint64_t) == sizeof(unsigned long long));

        ECC::NoLeak<ECC::uintBig> seed;
        auto buf = ufo::decodeMnemonic(words);
        ECC::Hash::Processor() << ufo::Blob(buf.data(), (uint32_t)buf.size()) >> seed.V;
...
EMSCRIPTEN_BINDINGS() 
{
    ...
    class_<KeyKeeper>("KeyKeeper")
        .constructor<const ufo::WordList&>()
