...
namespace ufo
{
    ...
    void HWWallet::getOwnerKey(Result<std::string> callback) const
    {
        ...
        trezor->call_UfoGetOwnerKey(true, [&m_runningFlag, &result](const Message& msg, std::string session, size_t queue_size)
        {
            result = child_cast<Message, hw::trezor::messages::ufo::UfoOwnerKey>(msg).key();
            m_runningFlag.clear();
        });
    ...
    void HWWallet::generateNonce(uint8_t slot, Result<ECC::Point> callback) const
    {
        ...
        trezor->call_UfoGenerateNonce(slot, [&m_runningFlag, &result](const Message& msg, std::string session, size_t queue_size)
            {
                result.m_X = ufo::Blob(child_cast<Message, hw::trezor::messages::ufo::UfoECCPoint>(msg).x().c_str(), 32);
                result.m_Y = child_cast<Message, hw::trezor::messages::ufo::UfoECCPoint>(msg).y();
                m_runningFlag.clear();
            });
    ...
    void HWWallet::getNoncePublic(uint8_t slot, Result<ECC::Point> callback) const
    {
        ...
        trezor->call_UfoGetNoncePublic(slot, [&m_runningFlag, &result](const Message& msg, std::string session, size_t queue_size)
            {
                result.m_X = ufo::Blob(child_cast<Message, hw::trezor::messages::ufo::UfoECCPoint>(msg).x().c_str(), 32);
                result.m_Y = child_cast<Message, hw::trezor::messages::ufo::UfoECCPoint>(msg).y();
                m_runningFlag.clear();
    ...
    void HWWallet::generateKey(const ECC::Key::IDV& idv, bool isCoinKey, Result<ECC::Point> callback) const
    {
        ...
        trezor->call_UfoGenerateKey(idv.m_Idx, idv.m_Type, idv.m_SubIdx, idv.m_Value, isCoinKey, [&m_runningFlag, &result](const Message& msg, std::string session, size_t queue_size)
            {
                result.m_X = ufo::Blob(child_cast<Message, hw::trezor::messages::ufo::UfoECCPoint>(msg).x().c_str(), 32);
                result.m_Y = child_cast<Message, hw::trezor::messages::ufo::UfoECCPoint>(msg).y();
                m_runningFlag.clear();
    ...
    void HWWallet::generateRangeProof(const ECC::Key::IDV& idv, bool isCoinKey, Result<ECC::RangeProof::Confidential> callback) const
    {
        ...
        trezor->call_UfoGenerateRangeproof(idv.m_Idx, idv.m_Type, idv.m_SubIdx, idv.m_Value, isCoinKey, [&m_runningFlag, &result](const Message& msg, std::string session, size_t queue_size)
            {
                const uint8_t* rp_raw = reinterpret_cast<const uint8_t*>(child_cast<Message, hw::trezor::messages::ufo::UfoRangeproofData>(msg).data().c_str());
                rangeproof_confidential_packed_t rp;
                memcpy(&rp, rp_raw, sizeof(rangeproof_confidential_packed_t));

                {
                    result.m_Part1.m_A.m_X = ufo::Blob(rp.part1.a.x, 32);
                    result.m_Part1.m_A.m_Y = rp.part1.a.y;

                    result.m_Part1.m_S.m_X = ufo::Blob(rp.part1.s.x, 32);
                    result.m_Part1.m_S.m_Y = rp.part1.s.y;
                }

                {
                    result.m_Part2.m_T1.m_X = ufo::Blob(rp.part2.t1.x, 32);
                    result.m_Part2.m_T1.m_Y = rp.part2.t1.y;

                    result.m_Part2.m_T2.m_X = ufo::Blob(rp.part2.t2.x, 32);
                    result.m_Part2.m_T2.m_Y = rp.part2.t2.y;
                }

                {
                    result.m_P_Tag.m_pCondensed[0].m_Value = ufo::Blob(rp.p_tag.condensed[0], 32);
                    result.m_P_Tag.m_pCondensed[1].m_Value = ufo::Blob(rp.p_tag.condensed[1], 32);
                }
                ...
                for (unsigned int c = 0; c < INNER_PRODUCT_N_CYCLES; c++)
                {
                    for (int i = 0; i < 2; i++)
                    {
                        result.m_P_Tag.m_pLR[c][i].m_X = ufo::Blob(rp.p_tag.LR[c][i].x, 32);
                        result.m_P_Tag.m_pLR[c][i].m_Y = rp.p_tag.LR[c][i].y;
                    }
                }

                {
                    result.m_Mu.m_Value = ufo::Blob(rp.mu, sizeof rp.mu);
                    result.m_tDot.m_Value = ufo::Blob(rp.tDot, 32);
                    result.m_Part3.m_TauX.m_Value = ufo::Blob(rp.part3.tauX, 32);
                }
    ...
    void HWWallet::signTransaction(const std::vector<Key::IDV>& inputs, const std::vector<Key::IDV>& outputs, const TxData& tx, Result<ECC::Scalar> callback) const
    {
        ...
        trezor->call_UfoSignTransaction(_inputs, _outputs, _tx, [&m_runningFlag, &result](const Message& msg, std::string session, size_t queue_size)
            {
                result.m_Value = ufo::Blob(child_cast<Message, hw::trezor::messages::ufo::UfoSignedTransaction>(msg).signature().c_str(), 32);
