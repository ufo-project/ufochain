...
int main()
{
	//auto logger = ufo::Logger::create(LOG_LEVEL_DEBUG, LOG_LEVEL_DEBUG);
	ufo::PrintEmissionSchedule();

	ufo::Rules::get().AllowPublicUtxos = true;
	ufo::Rules::get().FakePoW = true;
	ufo::Rules::get().MaxRollback = 10;
	...
	ufo::Rules::get().UpdateChecksum();

	ufo::PrepareTreasury();

	ufo::TestHalving();
	ufo::TestChainworkProof();
	...
	verify_test(ufo::helpers::ProcessWideLock("/tmp/UFO_node_test_lock"));

	ufo::DeleteFile(ufo::g_sz);
	ufo::DeleteFile(ufo::g_sz2);
...

	{
		...
		std::vector<ufo::BlockPlus::Ptr> blockChain;
		ufo::TestNodeProcessor1(blockChain);
		ufo::DeleteFile(ufo::g_sz);
		ufo::DeleteFile(ufo::g_sz2);
		...
		ufo::TestNodeProcessor2(blockChain);
		ufo::DeleteFile(ufo::g_sz);

		printf("NodeProcessor test3...\n");
		fflush(stdout);

		ufo::TestNodeProcessor3(blockChain);
		ufo::DeleteFile(ufo::g_sz);
		ufo::DeleteFile(ufo::g_sz2);
	}
