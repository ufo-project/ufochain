...
bool NodeProcessor::HandleBlock(const NodeDB::StateID& sid, MultiblockContext& mbc)
{
	...
	if (bFirstTime)
	{
		...
		if (m_Cursor.m_DifficultyNext.nBitsPow != s.m_PoW.m_Difficulty.nBitsPow)
		{
	...
	if (bFirstTime && bOk)
	{
		...
		if (bOk)
		{
            ...
            UFO_VERIFY(HandleValidatedBlock(block.get_Reader(), block, sid.m_Height, false));
	}
...
bool NodeProcessor::GenerateNewBlock(BlockContext& bc)
{
    ...
    UFO_VERIFY(HandleValidatedTx(bc.m_Block.get_Reader(), h, false)); // undo changes
