...
WalletModel::WalletModel(IWalletDB::Ptr walletDB, IPrivateKeyKeeper::Ptr keyKeeper, const std::string& nodeAddr, ufo::io::Reactor::Ptr reactor)
    : WalletClient(walletDB, nodeAddr, reactor, keyKeeper)
{
    qRegisterMetaType<ufo::wallet::WalletStatus>("ufo::wallet::WalletStatus");
    qRegisterMetaType<ufo::wallet::ChangeAction>("ufo::wallet::ChangeAction");
    qRegisterMetaType<vector<ufo::wallet::TxDescription>>("std::vector<ufo::wallet::TxDescription>");
    qRegisterMetaType<vector<ufo::wallet::SwapOffer>>("std::vector<ufo::wallet::SwapOffer>");
    qRegisterMetaType<ufo::Amount>("ufo::Amount");
    qRegisterMetaType<vector<ufo::wallet::Coin>>("std::vector<ufo::wallet::Coin>");
    qRegisterMetaType<vector<ufo::wallet::WalletAddress>>("std::vector<ufo::wallet::WalletAddress>");
    qRegisterMetaType<ufo::wallet::WalletID>("ufo::wallet::WalletID");
    qRegisterMetaType<ufo::wallet::WalletAddress>("ufo::wallet::WalletAddress");
    qRegisterMetaType<ufo::wallet::ErrorType>("ufo::wallet::ErrorType");
    qRegisterMetaType<ufo::wallet::TxID>("ufo::wallet::TxID");
    qRegisterMetaType<ufo::wallet::TxParameters>("ufo::wallet::TxParameters");

    connect(this, SIGNAL(walletStatus(const ufo::wallet::WalletStatus&)), this, SLOT(setStatus(const ufo::wallet::WalletStatus&)));
    connect(this, SIGNAL(addressesChanged(bool, const std::vector<ufo::wallet::WalletAddress>&)),
            this, SLOT(setAddresses(bool, const std::vector<ufo::wallet::WalletAddress>&)));
}
...
QString WalletModel::GetErrorString(ufo::wallet::ErrorType type)
{
...
void WalletModel::onStatus(const ufo::wallet::WalletStatus& status)
{
    emit walletStatus(status);
}

void WalletModel::onTxStatus(ufo::wallet::ChangeAction action, const std::vector<ufo::wallet::TxDescription>& items)
{
...
void WalletModel::onChangeCalculated(ufo::Amount change)
{
    emit changeCalculated(change);
}

void WalletModel::onAllUtxoChanged(const std::vector<ufo::wallet::Coin>& utxos)
{
    emit allUtxoChanged(utxos);
}

void WalletModel::onAddresses(bool own, const std::vector<ufo::wallet::WalletAddress>& addrs)
{
    emit addressesChanged(own, addrs);
}

void WalletModel::onSwapOffersChanged(ufo::wallet::ChangeAction action, const std::vector<ufo::wallet::SwapOffer>& offers)
{
    emit swapOffersChanged(action, offers);
}

void WalletModel::onCoinsByTx(const std::vector<ufo::wallet::Coin>& coins)
{
}
...
void WalletModel::onGeneratedNewAddress(const ufo::wallet::WalletAddress& walletAddr)
{
...
void WalletModel::onChangeCurrentWalletIDs(ufo::wallet::WalletID senderID, ufo::wallet::WalletID receiverID)
{
...
void WalletModel::onWalletError(ufo::wallet::ErrorType error)
{
...
void WalletModel::onPaymentProofExported(const ufo::wallet::TxID& txID, const ufo::ByteBuffer& proof)
{
    string str;
    str.resize(proof.size() * 2);

    ufo::to_hex(str.data(), proof.data(), proof.size());
    emit paymentProofExported(txID, QString::fromStdString(str));
}

ufo::Amount WalletModel::getAvailable() const
{
    return m_status.available;
}

ufo::Amount WalletModel::getReceiving() const
{
    return m_status.receiving;
}

ufo::Amount WalletModel::getReceivingIncoming() const
{
    return m_status.receivingIncoming;
}

ufo::Amount WalletModel::getReceivingChange() const
{
    return m_status.receivingChange;
}

ufo::Amount WalletModel::getSending() const
{
    return m_status.sending;
}

ufo::Amount WalletModel::getMaturing() const
{
    return m_status.maturing;
}

ufo::Height WalletModel::getCurrentHeight() const
{
    return m_status.stateID.m_Height;
}

ufo::Block::SystemState::ID WalletModel::getCurrentStateID() const
{
    return m_status.stateID;
}

void WalletModel::setStatus(const ufo::wallet::WalletStatus& status)
{
...
void WalletModel::setAddresses(bool own, const std::vector<ufo::wallet::WalletAddress>& addrs)
{
    if (own)
    {
        m_addresses = addrs;
