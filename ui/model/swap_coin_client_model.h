...
class SwapCoinClientModel
    : public QObject
    , public ufo::bitcoin::Client
{
    Q_OBJECT
public:
    using Ptr = std::shared_ptr<SwapCoinClientModel>;

    SwapCoinClientModel(ufo::bitcoin::IBridgeHolder::Ptr bridgeHolder,
        std::unique_ptr<ufo::bitcoin::SettingsProvider> settingsProvider,
        ufo::io::Reactor& reactor);

    ufo::Amount getAvailable();
    ufo::bitcoin::Client::Status getStatus() const;
    bool canModifySettings() const;

signals:
    void gotStatus(ufo::bitcoin::Client::Status status);
    void gotBalance(const ufo::bitcoin::Client::Balance& balance);
...
private slots:
    void requestBalance();
    void setBalance(const ufo::bitcoin::Client::Balance& balance);
    void setStatus(ufo::bitcoin::Client::Status status);
