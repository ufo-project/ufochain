...
AppModel::AppModel(WalletSettings& settings)
    : m_settings{settings}
    , m_walletReactor(ufo::io::Reactor::create())
{
    assert(s_instance == nullptr);
    s_instance = this;
    m_nodeModel.start();
...
#if defined(UFO_HW_WALLET)
bool AppModel::createTrezorWallet(std::shared_ptr<ECC::HKdfPub> ownerKey, const ufo::SecString& pass)
{
...
bool AppModel::openWallet(const ufo::SecString& pass)
{
    assert(m_db == nullptr);
...
void AppModel::onWalledOpened(const ufo::SecString& pass)
{
...
void AppModel::startWallet()
{
    ...
    auto swapTransactionCreator = std::make_shared<ufo::wallet::AtomicSwapTransaction::Creator>(m_db);
    ...
    if (auto btcClient = getBitcoinClient(); btcClient)
    {
        ...
        auto btcSecondSideFactory = ufo::wallet::MakeSecondSideFactory<BitcoinSide, bitcoin::IBridge, bitcoin::ISettingsProvider>(bitcoinBridgeCreator, *btcClient);
    ...
    if (auto ltcClient = getLitecoinClient(); ltcClient)
    {
        ...
        auto ltcSecondSideFactory = ufo::wallet::MakeSecondSideFactory<LitecoinSide, bitcoin::IBridge, litecoin::ISettingsProvider>(litecoinBridgeCreator, *ltcClient);
...
void AppModel::onFailedToStartNode(ufo::wallet::ErrorType errorCode)
{
    m_nsc.disconnect();

    if (errorCode == ufo::wallet::ErrorType::ConnectionAddrInUse && m_wallet)
    {
        emit m_wallet->walletError(errorCode);
        return;
    }

    if (errorCode == ufo::wallet::ErrorType::TimeOutOfSync && m_wallet)
    {
...
bool AppModel::checkWalletPassword(const ufo::SecString& pass) const
{
