...
namespace
{
    ...
    const char* kIsAlowedUfoMWLink = "UFO_mw_links_allowed";
...
const char* WalletSettings::WalletCfg = "ufo-wallet.cfg";
...
bool WalletSettings::isAllowedUfoMWLinks() const
{
    Lock lock(m_mutex);
    return m_data.value(kIsAlowedUfoMWLink, false).toBool();
}

void WalletSettings::setAllowedUfoMWLinks(bool value)
{
    Lock lock(m_mutex);
    m_data.setValue(kIsAlowedUfoMWLink, value);
}
...
QStringList WalletSettings::getLocalNodePeers()
{
    ...
    {
        auto defaultPeers = ufo::getDefaultPeers();
        peers.clear();
...
void WalletSettings::reportProblem()
{
    ...
    QFile zipFile = m_appDataDir.filePath("ufo v" + QString::fromStdString(PROJECT_VERSION) 
...
std::shared_ptr<ufo::bitcoin::Settings> WalletSettings::getBitcoinSettings() const
{
    std::string btcNodeUri, userName, password;
    ufo::Amount feeRate = 0;
    try
    {
        Lock lock(m_mutex);
        ...
        feeRate = m_data.value(kSwapBitcoinFeeRate).value<ufo::Amount>();
    }
    catch (...)
    {
        return {};
    ...
    ufo::bitcoin::BitcoinCoreSettings bitcoindSettings;
    ...
    auto btcSettings = make_shared<ufo::bitcoin::Settings>();
