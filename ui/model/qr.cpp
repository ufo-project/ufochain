...
QR::QR(const QString& addr, uint width, uint height, ufo::Amount amount)
    : m_addr(addr)
    , m_width(width)
    , m_height(height)
...
void QR::setAmount(ufo::Amount amount)
{
    m_amountGrothes = amount;
    update();
}
...
void QR::update()
{
    ...
    {
        query.addQueryItem("amount", ufoui::AmountToString(m_amountGrothes));
    }
