...
AddressItem::AddressItem(const ufo::wallet::WalletAddress& address)
...
{
    return ufoui::toString(m_walletAddress.m_walletID);
}
...
ufo::Timestamp AddressItem::getCreateTimestamp() const
{
    return m_walletAddress.getCreateTime();
}

ufo::Timestamp AddressItem::getExpirationTimestamp() const
{
    return m_walletAddress.getExpirationTime();
}

ContactItem::ContactItem(const ufo::wallet::WalletAddress& address)
    : m_walletAddress(address)
{
...
{
    connect(&m_model,
            SIGNAL(addressesChanged(bool, const std::vector<ufo::wallet::WalletAddress>&)),
            SLOT(onAddresses(bool, const std::vector<ufo::wallet::WalletAddress>&)));
...
void AddressBookViewModel::onAddresses(bool own, const std::vector<ufo::wallet::WalletAddress>& addresses)
{
    if (own)
    {
