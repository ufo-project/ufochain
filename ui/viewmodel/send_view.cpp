...
{
    LOG_INFO() << "ret Send amount grothes: " << _sendAmountGrothes << " 2ui: " << ufoui::AmountToString(_sendAmountGrothes).toStdString();
    return ufoui::AmountToString(_sendAmountGrothes);
}

void SendViewModel::setSendAmount(QString value)
{
    ufo::Amount amount = ufoui::StringToAmount(value);
    if (amount != _sendAmountGrothes)
    {
        _sendAmountGrothes = amount;
        LOG_INFO() << "Send amount: " << _sendAmountGrothes << " Coins: " << (long double)_sendAmountGrothes / ufo::Rules::Coin;
        _walletModel.getAsync()->calcChange(calcTotalAmount());
...
ufo::Amount SendViewModel::calcTotalAmount() const
{
...
{
    return  ufoui::AmountToString(isEnough() ? _walletModel.getAvailable() - calcTotalAmount() - _changeGrothes : 0);
}

QString SendViewModel::getMissing() const
{
    return ufoui::AmountToString(calcTotalAmount() - _walletModel.getAvailable());
}
...
void SendViewModel::onChangeCalculated(ufo::Amount change)
{
...
{
    return ufoui::AmountToString(_changeGrothes);
}

QString SendViewModel::getTotalUTXO() const
{
    return ufoui::AmountToString(calcTotalAmount() + _changeGrothes);
}
...
bool SendViewModel::canSend() const
{
    return !QMLGlobals::isSwapToken(_receiverTA) && getRreceiverTAValid()
           ...
           && QMLGlobals::isFeeOK(_feeGrothes, Currency::CurrUfo);
}
...
void SendViewModel::sendMoney()
{
    ...
    if(canSend())
    {
        ...
        auto p = ufo::wallet::CreateSimpleTransactionParameters()
            .SetParameter(ufo::wallet::TxParameterID::PeerID,  *_txParameters.GetParameter<ufo::wallet::WalletID>(ufo::wallet::TxParameterID::PeerID))
            .SetParameter(ufo::wallet::TxParameterID::Amount,  _sendAmountGrothes)
            .SetParameter(ufo::wallet::TxParameterID::Fee,     _feeGrothes)
            .SetParameter(ufo::wallet::TxParameterID::Message, ufo::ByteBuffer(messageString.begin(), messageString.end()));
...
{
    auto txParameters = ufo::wallet::ParseParameters(_receiverTA.toStdString());
    if (!txParameters)
    {
        return;
    }
    ...
    if (auto amount = _txParameters.GetParameter<ufo::Amount>(ufo::wallet::TxParameterID::Amount); amount)
    {
        setSendAmount(ufoui::AmountToString(*amount));
    }
    if (auto fee = _txParameters.GetParameter<ufo::Amount>(ufo::wallet::TxParameterID::Fee); fee)
    {
        setFeeGrothes(*fee);
    }
    if (auto comment = _txParameters.GetParameter(ufo::wallet::TxParameterID::Message); comment)
    {
