...
class TrezorThread : public QThread
{
    Q_OBJECT
public:
    TrezorThread(std::shared_ptr<ufo::HWWallet> hw);
...
private:
    std::shared_ptr<ufo::HWWallet> m_hw;
};
...
public:
    ...
    using DoneCallback = std::function<bool (ufo::wallet::IWalletDB::Ptr db, const std::string& walletPass)>;
...
private:
    ...
    ufo::WordList m_generatedPhrases;
...
#if defined(UFO_HW_WALLET)
    std::shared_ptr<ufo::HWWallet> m_hwWallet;
