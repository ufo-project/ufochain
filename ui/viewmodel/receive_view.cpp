...
void ReceiveViewModel::onGeneratedNewAddress(const ufo::wallet::WalletAddress& addr)
{
    _receiverAddress = addr;
    emit receiverAddressChanged();

    _qr->setAddr(ufoui::toString(_receiverAddress.m_walletID));
    updateTransactionToken();
}
...
{
    _txParameters.SetParameter(ufo::wallet::TxParameterID::Amount, _amountToReceiveGrothes);
    _txParameters.SetParameter(ufo::wallet::TxParameterID::PeerID, _receiverAddress.m_walletID);
    _txParameters.SetParameter(ufo::wallet::TxParameterID::TransactionType, ufo::wallet::TxType::Simple);
