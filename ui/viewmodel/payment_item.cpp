...
void PaymentInfoItem::setPaymentProof(const QString& value)
{
    if (m_paymentProof != value)
    {
        ...
        try
        {
            m_paymentInfo = ufo::wallet::storage::PaymentInfo::FromByteBuffer(ufo::from_hex(m_paymentProof.toStdString()));
        ...
        : PaymentInfoItem(parent)
{
    ...
    connect(model.get(), SIGNAL(paymentProofExported(const ufo::wallet::TxID&, const QString&)), SLOT(onPaymentProofExported(const ufo::wallet::TxID&, const QString&)));
    model->getAsync()->exportPaymentProof(txID);
}

void MyPaymentInfoItem::onPaymentProofExported(const ufo::wallet::TxID& txID, const QString& proof)
{
