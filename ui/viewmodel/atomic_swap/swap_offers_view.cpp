...
{
    connect(&m_walletModel, SIGNAL(availableChanged()), this, SIGNAL(ufoAvailableChanged()));
    connect(&m_walletModel,
            SIGNAL(txStatus(ufo::wallet::ChangeAction, const std::vector<ufo::wallet::TxDescription>&)),
            SLOT(onTransactionsDataModelChanged(ufo::wallet::ChangeAction, const std::vector<ufo::wallet::TxDescription>&)));

    connect(&m_walletModel,
            SIGNAL(swapOffersChanged(ufo::wallet::ChangeAction, const std::vector<ufo::wallet::SwapOffer>&)),
            SLOT(onSwapOffersDataModelChanged(ufo::wallet::ChangeAction, const std::vector<ufo::wallet::SwapOffer>&)));
...
QString SwapOffersViewModel::ufoAvailable() const
{
    return ufoui::AmountToString(m_walletModel.getAvailable());
}

QString SwapOffersViewModel::btcAvailable() const
{
    return ufoui::AmountToString(m_btcClient->getAvailable());
}

QString SwapOffersViewModel::ltcAvailable() const
{
    return ufoui::AmountToString(m_ltcClient->getAvailable());
}

QString SwapOffersViewModel::qtumAvailable() const
{
    return ufoui::AmountToString(m_qtumClient->getAvailable());
}
...
void SwapOffersViewModel::onTransactionsDataModelChanged(ufo::wallet::ChangeAction action, const std::vector<ufo::wallet::TxDescription>& transactions)
{
...
void SwapOffersViewModel::onSwapOffersDataModelChanged(ufo::wallet::ChangeAction action, const std::vector<ufo::wallet::SwapOffer>& offers)
{
    ...
    for (const auto& offer : offers)
    {
        ...
        auto peerResponseTime = offer.GetParameter<ufo::Height>(ufo::wallet::TxParameterID::PeerResponseTime);
        auto minHeight = offer.GetParameter<ufo::Height>(ufo::wallet::TxParameterID::MinHeight);
        ...
        if (currentHeight && peerResponseTime && minHeight)
        {
            ...
            timeExpiration = ufoui::CalculateExpiresTime(currentHeight, expiresHeight);
        }
...
{
    return hasActiveTx(toStdString(ufoui::Currencies::Bitcoin));
}

bool SwapOffersViewModel::hasLtcTx() const
{
    return hasActiveTx(toStdString(ufoui::Currencies::Litecoin));
}

bool SwapOffersViewModel::hasQtumTx() const
{
    return hasActiveTx(toStdString(ufoui::Currencies::Qtum));
}
