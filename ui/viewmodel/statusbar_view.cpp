...

{
    ...
    connect(&m_model, SIGNAL(walletError(ufo::wallet::ErrorType)),
        SLOT(onGetWalletError(ufo::wallet::ErrorType)));
    ...
    connect(&AppModel::getInstance().getNode(), SIGNAL(failedToSyncNode(ufo::wallet::ErrorType)),
            SLOT(onGetWalletError(ufo::wallet::ErrorType)));
...
void StatusbarViewModel::onGetWalletError(ufo::wallet::ErrorType error)
{
    setIsOnline(false);
