...
SendSwapViewModel::SendSwapViewModel()
    ...
    , _sendFeeGrothes(0)
    , _sendCurrency(Currency::CurrUfo)
...
namespace
{
    Currency convertSwapCoinToCurrency(ufo::wallet::AtomicSwapCoin  coin)
    {
        switch (coin)
        {
        ...
        default:
            return Currency::CurrUfo;
        }
    }
}

void SendSwapViewModel::fillParameters(ufo::wallet::TxParameters parameters)
{
    // Set currency before fee, otherwise it would be reset to default fee
    using namespace ufo::wallet;
    using namespace ufo;

    auto isUfoSide = parameters.GetParameter<bool>(TxParameterID::AtomicSwapIsUfoSide);
    auto swapCoin = parameters.GetParameter<AtomicSwapCoin>(TxParameterID::AtomicSwapCoin);
    auto ufoAmount = parameters.GetParameter<Amount>(TxParameterID::Amount);
    ...
    if (peerID && swapAmount && ufoAmount && swapCoin && isUfoSide
        && peerResponseTime && offeredTime && minHeight)
    {
        if (*isUfoSide) // other participant is not a ufo side
        {
            // Do not set fee, it is set automatically based on the currency param
            setSendCurrency(Currency::CurrUfo);
            setSendAmount(ufoui::AmountToString(*ufoAmount));
            setReceiveCurrency(convertSwapCoinToCurrency(*swapCoin));
            setReceiveAmount(ufoui::AmountToString(*swapAmount));
        }
        else
        {
            // Do not set fee, it is set automatically based on the currency param
            setSendCurrency(convertSwapCoinToCurrency(*swapCoin));
            setSendAmount(ufoui::AmountToString(*swapAmount));
            setReceiveCurrency(Currency::CurrUfo);
            setReceiveAmount(ufoui::AmountToString(*ufoAmount));
        }
        ...
        setExpiresTime(ufoui::CalculateExpiresTime(currentHeight, expiresHeight));
...
void SendSwapViewModel::setParameters(QVariant parameters)
{
    ...
    {
        auto p = parameters.value<ufo::wallet::TxParameters>();
        fillParameters(p);
    }
}
...
void SendSwapViewModel::setToken(const QString& value)
{
    if (_token != value)
    {
        _token = value;
        emit tokenChanged();
        auto parameters = ufo::wallet::ParseParameters(_token.toStdString());
...
{
    auto type = _txParameters.GetParameter<ufo::wallet::TxType>(ufo::wallet::TxParameterID::TransactionType);
    return type && *type == ufo::wallet::TxType::AtomicSwap;
}

QString SendSwapViewModel::getSendAmount() const
{
    return ufoui::AmountToString(_sendAmountGrothes);
}

void SendSwapViewModel::setSendAmount(QString value)
{
    const auto amount = ufoui::StringToAmount(value);
...
{
    return ufoui::AmountToString(_receiveAmountGrothes);
}
...
void SendSwapViewModel::onChangeCalculated(ufo::Amount change)
{
...
bool SendSwapViewModel::isEnough() const
{
    ...
    case Currency::CurrBtc:
    {
        ...
        const ufo::Amount total = _sendAmountGrothes + _sendFeeGrothes;
...
{
    auto peerID = _txParameters.GetParameter<ufo::wallet::WalletID>(ufo::wallet::TxParameterID::PeerID);
    if (peerID)
    {
        return ufoui::toString(*peerID);
    }
    return _token;
}
...
{
    using ufo::wallet::TxParameterID;
    
    auto txParameters = ufo::wallet::TxParameters(_txParameters);
    auto isUfoSide = txParameters.GetParameter<bool>(TxParameterID::AtomicSwapIsUfoSide);
    auto ufoFee = (*isUfoSide) ? getSendFee() : getReceiveFee();
    auto swapFee = (*isUfoSide) ? getReceiveFee() : getSendFee();
    auto subTxID = isUfoSide ? ufo::wallet::SubTxIndex::REDEEM_TX : ufo::wallet::SubTxIndex::LOCK_TX;

    txParameters.SetParameter(TxParameterID::Fee, ufo::Amount(ufoFee));
    txParameters.SetParameter(TxParameterID::Fee, ufo::Amount(swapFee), subTxID);
    if (!_comment.isEmpty())
    {
...

    {
        ...
        auto swapCoin = txParameters.GetParameter<ufo::wallet::AtomicSwapCoin>(TxParameterID::AtomicSwapCoin);
        auto amount = txParameters.GetParameter<ufo::Amount>(TxParameterID::Amount);
        auto swapAmount = txParameters.GetParameter<ufo::Amount>(TxParameterID::AtomicSwapAmount);
        auto responseHeight = txParameters.GetParameter<ufo::Height>(TxParameterID::PeerResponseTime);
        auto minimalHeight = txParameters.GetParameter<ufo::Height>(TxParameterID::MinHeight);

        LOG_INFO() << *txID << " Accept offer.\n\t"
                    << "isUfoSide: " << (*isUfoSide ? "true" : "false") << "\n\t"
