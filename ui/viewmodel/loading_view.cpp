...
LoadingViewModel::LoadingViewModel()
    ...
    , m_lastProgress{0.}
    , m_bpsWholeTimeFilter(std::make_unique<ufoui::Filter>(kFilterRange))
    , m_bpsWindowedFilter(std::make_unique<ufoui::Filter>(kFilterRange * 3))
    , m_estimateFilter(std::make_unique<ufoui::Filter>(kFilterRange))
    , m_startTimestamp{0}
    ...
    , m_bpsRecessionCount{0}
{
    ...
    connect(&m_walletModel, SIGNAL(walletError(ufo::wallet::ErrorType)), SLOT(onGetWalletError(ufo::wallet::ErrorType)));
...
void LoadingViewModel::resetWallet()
{
    ...
    disconnect(&m_walletModel, SIGNAL(walletError(ufo::wallet::ErrorType)), this, SLOT(onGetWalletError(ufo::wallet::ErrorType)));
...
void LoadingViewModel::onGetWalletError(ufo::wallet::ErrorType error)
{
    if (m_isCreating)
    {
        switch (error)
        {
            case ufo::wallet::ErrorType::NodeProtocolIncompatible:
            {
                //% "Incompatible peer"
                emit walletError(qtTrId("loading-view-protocol-error"), m_walletModel.GetErrorString(error));
                return;
            }
            case ufo::wallet::ErrorType::ConnectionAddrInUse:
            case ufo::wallet::ErrorType::ConnectionRefused:
            case ufo::wallet::ErrorType::HostResolvedError:
            {
                //% "Connection error"
