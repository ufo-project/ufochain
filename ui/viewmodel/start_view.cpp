...
namespace
{
    ...
    void removeNodeDataIfNeeded()
    {
        try
        {
            ...
            catch (const ufo::NodeDBUpgradeException&)
            {
            }
...
#if defined(UFO_HW_WALLET)
    , m_hwWallet(make_shared<ufo::HWWallet>())
    , m_trezorTimer(this)
...
TrezorThread::TrezorThread(ufo::HWWallet::Ptr hw)
    : m_hw(hw)
{
...
const QList<QObject*>& StartViewModel::getRecoveryPhrases()
{
    if (m_recoveryPhrases.empty())
    {
        if (!m_isRecoveryMode)
        {
            m_generatedPhrases = ufo::createMnemonic(ufo::getEntropy(), ufo::language::en);
        }
        else
        {
...
bool StartViewModel::createWallet()
{
    ...
    auto buf = ufo::decodeMnemonic(m_generatedPhrases);
