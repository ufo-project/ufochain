...
UtxoItem::UtxoItem(const ufo::wallet::Coin& coin)
    : _coin{ coin }
{
...
{
    return AmountToString(_coin.m_ID.m_Value, Currencies::Ufo);
}
...
ufo::Amount UtxoItem::rawAmount() const
{
    return _coin.m_ID.m_Value;
}

const ufo::wallet::Coin::ID& UtxoItem::get_ID() const
{
	return _coin.m_ID;
}

ufo::Height UtxoItem::rawMaturity() const
{
...
{
    connect(&_model, SIGNAL(allUtxoChanged(const std::vector<ufo::wallet::Coin>&)),
        SLOT(onAllUtxoChanged(const std::vector<ufo::wallet::Coin>&)));
...
{
    return QString(ufo::to_hex(_model.getCurrentStateID().m_Hash.m_pData, 10).c_str());
}
...
void UtxoViewModel::onAllUtxoChanged(const std::vector<ufo::wallet::Coin>& utxos)
{
