...
{
    QMessageBox::information(nullptr, "UfoWalletUI", message);
}
...
{
    return ufo::wallet::check_receiver_address(text.toStdString());
}
...
bool QMLGlobals::isTransactionToken(const QString& text)
{
    ...
    auto params = ufo::wallet::ParseParameters(text.toStdString());
    return params && params->GetParameter<ufo::wallet::TxType>(ufo::wallet::TxParameterID::TransactionType);
}
...
bool QMLGlobals::isSwapToken(const QString& text)
{
    ...
    auto type = params->GetParameter<ufo::wallet::TxType>(ufo::wallet::TxParameterID::TransactionType);
...
bool QMLGlobals::isFeeOK(uint32_t fee, Currency currency)
{
    switch (currency)
    {
    case Currency::CurrUfo: return fee >= minFeeUfo();
...
{
    ufo::SecString secretPass = value.toStdString();
    return AppModel::getInstance().checkWalletPassword(secretPass);
}

QString QMLGlobals::ufoFeeRateLabel()
{
    return "GROTH";
}
...
{
    switch (currency) {
        case Currency::CurrUfo: return minFeeUfo();
...
QString QMLGlobals::rawTxParametrsToTokenStr(QVariant variantTxParams)
{
    ...
    {
        auto txParameters = variantTxParams.value<ufo::wallet::TxParameters>();
