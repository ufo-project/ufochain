...
public:
    ...
    TxObject(const ufo::wallet::TxDescription& tx, QObject* parent = nullptr);

    auto timeCreated() const -> QDateTime;
    auto getTxID() const -> ufo::wallet::TxID;
    ...
    void setStatus(ufo::wallet::TxStatus status);
    void setFailureReason(ufo::wallet::TxFailureReason reason);
    void update(const ufo::wallet::TxDescription& tx);
...
protected:
    auto getTxDescription() const -> const ufo::wallet::TxDescription&;
 
    ufo::wallet::TxDescription m_tx;
    QString m_kernelID;
    ufo::wallet::TxType m_type;
};
