...
public:
    ...
    UtxoItem() = default;
    UtxoItem(const ufo::wallet::Coin& coin);
    virtual ~UtxoItem();
    ...
    ufo::Amount rawAmount() const;
    ufo::Height rawMaturity() const;
	const ufo::wallet::Coin::ID& get_ID() const;

signals:
    void changed();

private:
    ufo::wallet::Coin _coin;
};
...
public slots:
    void onAllUtxoChanged(const std::vector<ufo::wallet::Coin>& utxos);
signals:
    void allUtxoChanged();
    void stateChanged();
private:
